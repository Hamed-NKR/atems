...
function [Imgs, imgs, pixsize] = load_imgs_v3(fd, n, opts)
...
% field for manual assigning of scale bar
if ~isfield(opts, 'manu')
    opts.manu = 'on';
end
%-------------------------------------------------------------------------%
...
Imgs = detect_footer_scale(Imgs, f_replace, opts);
...
function [Imgs, pixsizes] = detect_footer_scale(Imgs, f_replace, opts)
...
for jj=1:length(Imgs)
    ...
    sc_found = 0; % % flag whether scale was found
    ...
    if footer_found == 0
        ...
        if ~isempty(sc_end) % if text found
            sc_found = 1; % mark that text has been found
    ...
    opts_manu = opts.manu;
    if sc_found == 0
        if ismember(opts_manu, {'ON', 'On', 'on'})
            Imgs(jj).pixsize = tools.ui_scale_bar(Imgs(jj).raw); % try to manually extract the scale bar
        else
